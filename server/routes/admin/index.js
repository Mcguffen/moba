...
module.exports = app => {
  ...
  // 获取创建分类的接口
    ...
    router.delete('/categories/:id', async (req, res) => {
      // 使用模型创建数据Category.create（）参数也就是数据来源是客户端发来的表单提交数据req.body 要想使用 还得去/server/index.js添加一个中间件
      model = await Category.findByIdAndDelete(req.params.id, req.body)
      res.send({
        success: true
    ...
    // 获取分类列表的接口
      ...
      router.get('/categories', async (req, res) => {
        // 使用模型创建数据Category.find（）参数也就是数据来源是客户端发来的表单提交数据 要想使用 还得去/server/index.js添加一个中间件,populate('parent')可以查出parent的关联字段
